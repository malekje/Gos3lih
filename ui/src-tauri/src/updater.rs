...
pub async fn apply_update(download_url: &str) -> Result<()> {
    ...
    let update_exe = parent.join("gos3lih.update.exe");
    let backup_exe = parent.join("gos3lih.old.exe");
    ...
    for i in 0..archive.len() {
        ...
        let dest = if name.ends_with(".exe") && name.contains("gos3lih") {
            update_exe.clone()
        } else {
            parent.join(
    ...
    info!(
        "Update extracted ({} bytes), scheduling restart…",
        bytes.len()
    );
